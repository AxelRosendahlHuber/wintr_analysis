# run the analysis for Hartwig files
library(dndscv)
library(tidyverse)
library(rtracklayer)
...
library(data.table)
library(patchwork)
source("~/repos/wintr/R/dndscv_5.R")

setwd("~/wintr/hartwig_tests/")

# loop over different cohorts
hartwig_files = list.files("~/wintr/hartwig_tests/hartwig_muts/", pattern = "txt", full.names = TRUE)
names(hartwig_files) = gsub("txt", "", basename(hartwig_files))

day = as.Date(Sys.time(), "%Y%b%e") |> as.character()
plotdir = paste0("~/wintr/hartwig_tests/hartwig_", day, "/")
if (!dir.exists(plotdir)) {dir.create(plotdir)}

args = commandArgs(trailingOnly = TRUE) |> as.numeric()

print(args)
hartwig_files = hartwig_files[args[1]]
print(hartwig_files)
print(names(hartwig_files))

for (name in names(hartwig_files)) {
  print(name)
  hartwig_file = hartwig_files[name]
  
  mutations = data.table::fread(hartwig_file)
  
  dndscv_vanilla = dndscv(mutations)
  
  # read the dndscv-intron files
  RefCDS_wgs = readRDS("~/repos/wintr/data/RefCDS_wgs.rds")
  gr_genes = readRDS("~/repos/wintr/data/gr_genes.rds")
  gr_transcripts = readRDS("~/repos/wintr/data/gr_transcripts.rds")
  sm = read.delim("~/repos/wintr/data/substmodel_introns.tsv") |> 
    as.matrix()
  ...
  cgc_genes = read.delim("~/repos/wintr/data/Census_allTue Oct 15 11_40_02 2024.tsv")$Gene.Symbol
